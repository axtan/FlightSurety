...
contract('Oracles', async (accounts) => {
  ...
  it('can request flight status', async () => {
    ...
    let timestamp = Math.floor(Date.now() / 1000) + (24 * 60 * 60 * 3);

    await config.flightSuretyData.authorizeCaller(config.flightSuretyApp.address,{from:accounts[0]});
    await config.flightSuretyApp.fundAirline({from:config.firstAirline,value:10*Math.pow(10, 18)});
    ...
    result = await config.flightSuretyApp.fetchFlightStatus(config.firstAirline, flight, timestamp);
    ...
    for(let a=1; a<TEST_ORACLES_COUNT; a++) {
      ...
      for(let idx=0;idx<3;idx++) {
        ...
        try {
          ...
          console.log('OK',oracleIndexes[idx].toNumber(), config.firstAirline, flight, timestamp, STATUS_CODE_ON_TIME);

        }
        catch(e) {
          // Enable this when debugging
           //console.log('\nError', idx, oracleIndexes[idx].toNumber(), flight, timestamp,e['reason']);
        }
